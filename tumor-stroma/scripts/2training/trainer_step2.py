...
def Accuracy_foreground(inp, targ, void_idx=3, axis=1):
    "Computes non-background accuracy for multiclass segmentation"
    #https://github.com/fastai/fastai/blob/aeb5317d11d986420fde85fc9749aac1f53eefd6/fastai/metrics.py#L323
    targ = cast(targ.squeeze(1), TensorBase)
    mask = targ != void_idx
...
metrics = [
    #MIOU(3, axis=1), 
    Accuracy_Overall,Accuracy_foreground,DiceMulti(axis=1), JaccardCoeff(axis=1)]
...
previous_fname= "unet-resnet34_before-unfreeze-WD-1_BS32_20220315_0838"
fname="unet-resnet34_after-unfreeze-WD-1_BS{}_{}".format(BS, timeStamp)
early_fname = 'best_'+fname

callbacks_After_Unfreeze=[CSVLogger(fname=previous_fname+'.csv', append=True) ,ShowGraphCallback(),
    EarlyStoppingCallback(monitor='valid_loss', min_delta=1e-4, patience=4),
    SaveModelCallback(monitor='valid_loss', comp=np.less, with_opt=True, min_delta=1e-4, fname=early_fname, every_epoch=False)]
...
#output = opj(outputDir,'reports',fname+'.txt')
#Path(output).touch()
#orig_stdout = sys.stdout
#f = open(output, 'w')
#sys.stdout = f

print('Training before unfreeze: \nBS32, \naug_xtrtfms\nWD1e-1\nfileName:'+fname+'\nCurrent date & time '+timeStamp)
...
learn.fit_flat_cos(20, slice(lr),cbs=callbacks_After_Unfreeze)

fig_path = opj(outputDir,'plots',fname+'.png')
plt.savefig(fig_path)
clear_pyplot_memory()

modelPath = opj(projectDir,'models',fname+'_stage-2')
...
res_plot_path = opj(outputDir,'plots',fname+'results_stage-2.png')
